...
parser.add_argument('--r', type=float, default=0.04,
                    help=' LLR regularization constant    [default: 0.4]')
...
if args.coil_batch_size is None:
    coils_str="all"
else:
    coils_str=args.coil_batch_size

print('>  coil_batch_size: ', coils_str)
print('>  Blk_shape: ', blk_shape_str)
print('>  Blk_strides: ', blk_strides_str)
...
# coil_file = outprefstr + '/coil' #Changed from coils_reord to coil #################################
...
f = h5py.File(outprefstr + '/kdat_slice_032.h5', 'r')
MB = f['MB'][()]
N_slices = f['Slices'][()]
...
print('>  N_slices_collap: ', N_slices_collap)
...
for s in slice_loop:
    ...
    print ("Kdat _prep shape (after splitting into shots):", kdat_prep.shape)
    ...
    if args.pi is True:
        ...
        with device:
            ...
            for d in range(N_diff):
                ...
                alg_method = sp.alg.ConjugateGradient(AHA, AHy, img, max_iter=1, verbose=True)
        ...
        print('dwi_pi_comb shape:', dwi_pi_comb)
        print('exiting')
        exit()
        ...
        print(">>>> Recon_file:", np.shape(recon_file))
    ...
    if (args.jets is True) and (N_segments == 1):
        ...
        for s in range(args.split):
            ...
            shot_llr = app.HighDimensionalRecon(kdat_prep_split, coil2,
                        ...
                        regu='LLR', blk_shape=args.blk_shape, blk_strides=args.blk_strides, coil_batch_size=args.coil_batch_size,
                        solver='ADMM', lamda=args.r, rho=0.05, max_iter=15,
                        show_pbar=False, verbose=True,
                        device=device).run()######################################## blk_shape
            ...
            recon_file = f"/jets_slice_000{split_str}"+ f"_Coilbch_{coils_st}" + f"_blk_{blk_shape_str}" + f"_r_{args.r}" + f"_str_{blk_strides_str}.h5" ## naming changed to suite comparison purposes ' ## naming changed to suite comparison purposes 
            print(">>>> Recon_file:", recon_file)
            f = h5py.File(outprefstr + recon_file , 'w')
        ...
        for s in range(args.split):
            ...
            if USE_SHOT_LLR is True:
                ...
                dwi_shot = app.HighDimensionalRecon(ksp_acs, mps_acs,
                                    ...
                                    combine_echo=False,
                                    regu='LLR', blk_shape=args.blk_shape, blk_strides=args.blk_strides,coil_batch_size=args.coil_batch_size,
                                    solver='ADMM', lamda=args.r, rho=rho, max_iter=15,
                                    show_pbar=False, verbose=True,
                                    device=device).run()##############################blk_shape
            ...
            recon_file = f"/jets_slice_000{split_str}"+ f"_Coilbch_{coils_str}" + f"_blk_{blk_shape_str}" + f"_r_{args.r}" + f"_str_{blk_strides_str}"+f"_red_{reduce_str} .h5" ## naming changed to suite comparison purposes 
            print(">>>> Recon_file:", recon_file)
            f = h5py.File(outprefstr + recon_file, 'w')
